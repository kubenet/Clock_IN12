...
int  serialRead = Serial.read();
   if (serialRead == 'A'){
int      servoAVal = Serial.parseInt();
     Clock.setHour(servoAVal);
   }
   if (serialRead == 'B'){
 int     servoBVal = Serial.parseInt();
     Clock.setMinute(servoBVal);
   }
}
  ...
  unsigned long currentTime = millis(); //текущее время с момента запуска рограммы
   if (currentTime>=(startTime2+200)) // если прошл14 200мс посмотрим какая кнопка была нажата
   {
      timeout=false;
